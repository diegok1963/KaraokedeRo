...
function renderTable() {
  ...
  if (page.length === 0) {
    ...
    for (var i = 0; i < page.length; i++) {
      ...
      html += '<div class="card-nombre" style="flex:1;min-width:0">' + titularBtn + ' Reserva de ' + esc(r.nombre) + '</div>';
      html += '<div style="display:flex;flex-direction:column;align-items:center;gap:2px;flex-shrink:0">';
      html += '<span style="font-size:.6rem;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Mesa</span>';
      html += '<input type="text" value="' + esc(r.mesa||'') + '" placeholder="—" ';
      html += 'onchange="updateMesa(' + r.id + ',this.value)" ';
      html += 'style="width:52px;background:rgba(192,38,211,.1);border:1px solid rgba(192,38,211,.4);border-radius:8px;padding:4px 6px;color:#e879f9;font-size:.85rem;font-weight:700;text-align:center;outline:none"/>';
      html += '</div></div></div>';
